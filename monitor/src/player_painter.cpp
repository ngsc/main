                        ...
                        const rcss::rcg::BallT & ball ) const
{
    ...
    //left is first team, right is second
    /*int team_id = (player.side_ == 'l' ? 0 : 1);
    PlayerControl::setPlayerPoint(param.player_type_.id_, team_id, param.x_, param.y_);
    *///color__
    drawBody( painter, param );
    // PlayerControl::setPlayerColor(param.player_type_.id_, team_id, painter.brush().color());
    //neck__
    drawDir( painter, param );
    // PlayerControl::setPlayerNeckAngle(param.player_type_.id_, team_id, param.player_.neck_);
    if ( player.hasNeck()
         && player.hasView()
         && opt.showViewArea() )
    {
