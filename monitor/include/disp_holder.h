...
#include <.\monitor\include\rcsslogplayer\types.h>
#include <.\monitor\include\rcsslogplayer\handler.h>
...
private:
	...
	std::pair<char, std::pair<char, int> > M_foul_card;
    ...
    std::string M_commentary;
	std::stringstream M_commentary_log;
...
public:
	...
	const std::pair<char, std::pair<char, int> > & FoulCard() const
	{
		return M_foul_card;
	}
...
private:
    void handleCommentaries( const std::string & msg );
	void handleStatistics(const std::string & msg);
	void analyzeTeamGraphic( const std::string & msg );
	void analyzeStatistic(DispPtr& disp);
	void handleFoulCards(const std::string & msg);
